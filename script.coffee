scrollet =
    ...
    prepareAlert: ->
        ...
        window.setTimeout 'scrollet.toggleAlert()', 100

    setMozDiff: (o) ->
        scrollet.initial_y ?= o.y
        diff = o.y - scrollet.initial_y
        if Math.abs(diff) > 0.05
            scrollet.real_diff = parseInt(-10*diff)
        else
            scrollet.real_diff = 0

    setDiff: (o) ->
        switch window.orientation
            when 90 then y_var = -0.5*o.gamma
            when -90 then y_var = 0.5*o.gamma
            else y_var = o.beta

        scrollet.initial_y ?= y_var
        diff = y_var - scrollet.initial_y

        if Math.abs(diff) > 5
            scrollet.real_diff = parseInt(-0.2*diff)
        else
            scrollet.real_diff = 0

    resetY: ->
        scrollet.initial_y = null

    setOrientation: (e) ->
        scrollet.resetY()

    moveAlert: ->
        scrollet.alert.style.top = (window.pageYOffset) + 'px'

    toggleAlert: ->
        if parseInt(scrollet.alert.style.opacity) is 1
